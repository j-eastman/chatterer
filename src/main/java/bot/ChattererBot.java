...
public class ChattererBot extends Bot {
	...
	String[] responses = new String[] { "Sick %s man! (JK LOL)", "You just sent me a %s message. It was probably lame.",
			"Wow, cool %s! (That totally wasn't sarcastic)", "That %s was lit.",
			"That %s better not be inappropriate! (As if I care lol)",
			"Yo bro (or bra) awesome %s! (LOL probably not)" };
	...
	public void onTextMessage(Message message) {
		...
		boolean doSend = true;
		String response = "";
		if (message.body != null && (message.body.toLowerCase().contains("oh hai mark")
				|| message.body.toLowerCase().contains("oh hi mark"))) {
			message.reply(new String[] { "http://blanket1aprons.x10host.com/source/ohhimark.mp4",
					String.valueOf(Message.TYPE_VIDEO) });
			doSend = false;
		}
		if (message.body != null && message.body.toLowerCase().contains("apart lisa")) {
			message.reply(new String[] { "http://blanket1aprons.x10host.com/source/tearingmeapart.mp4",
					String.valueOf(Message.TYPE_VIDEO) });
			doSend = false;
		}
