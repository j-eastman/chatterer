...
import se.supertransformer.kja.Utils;

public class ThreadNewConnection extends Thread
{
	private Socket socket;
	private KikApi kikApi;
	
	public ThreadNewConnection(Socket socket, KikApi kikApi)
	{
		this.socket = socket;
		this.kikApi = kikApi;
	}
	...
	public void run()
	{
		try
		{
			...
			{
				writer.print("Content-Type: " + Files.probeContentType(kikApi.getListenerThread().getFileMap().get(fileRequested).toPath()) + "\r\n");
			}
			...
			} else
			{
				...
				FileInputStream fileInputStream = new FileInputStream(kikApi.getListenerThread().getFileMap().get(fileRequested));
			...
			//writer.write("<form method='post' action='index.php'><input type='text' name='test'> <input type='submit' name='submit'></form>");
